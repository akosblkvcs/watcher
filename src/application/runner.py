...
class Runner:
    ...
    def _build_target_success_data(
        ...
        return TargetSuccessData(observation=observation, finished=finished)
    ...
    def _build_target_failure_data(
        ...
        return TargetFailureData(observation=observation, finished=finished)
    ...
    def _handle_target_success(self, target: Target, data: TargetSuccessData) -> None:
        ...
        target.last_raw_text = obs.raw_text
        target.last_processed_text = obs.processed_text
        target.last_error_message = None
        target.last_duration_ms = obs.duration_ms

        target_checked.send(self, target=target, observation=obs, success=True)
        if obs.changed:
            target_changed.send(self, target=target, observation=obs)
    ...
    def _handle_target_failure(self, target: Target, data: TargetFailureData) -> None:
        ...
        target.last_error_message = obs.error_message
        target.last_duration_ms = obs.duration_ms

        target_checked.send(self, target=target, observation=obs, success=False)
